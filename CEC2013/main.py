...
from CEC2013.runner import run_experiment, write_comparison_csv
from CEC2013.config import POP_SIZE, MAX_FES, RUNS, LOWER_BOUND, UPPER_BOUND
...
def _prompt_function():
    ...
    while True:
        raw = input("\n  Select function number [1-28]: ").strip()
        try:
            func_id = int(raw)
        except ValueError:
        ...
        if 1 <= func_id <= 28:
            return func_id
        print(f"  ✗ Function {func_id} is out of range. Must be 1-28.\n")


def main():
    parser = argparse.ArgumentParser(
        description="CEC2013 Benchmark Suite",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python -m CEC2013.main                          # Interactive menu
  python -m CEC2013.main --algo rao2 --func 1     # Run RAO-2 on F1
  python -m CEC2013.main --all                    # Run all algos
  python -m CEC2013.main --runs 10 --max-fes 50000  # Custom params
        """
    )
    parser.add_argument("--algo", choices=list(ALGORITHMS.keys()),
                        help="Algorithm to run")
    parser.add_argument("--func", type=int, choices=range(1, 29),
                        help="Function ID (1-28)", metavar="FUNC_ID")
    ...
    # CEC2013 supports D=2,5,10,20,30,40,50
    dims_to_run = [2, 10, 20, 30, 50]
    ...
    if len(algo_names) == len(list(ALGORITHMS.keys())):
        ...
        print("=" * 60)
        from CEC2013.summarize import build_summary
        build_summary()
